	...
	function XD_Vector2(_x = 0, _y = 0) constructor {
		...
		static set_magnitude		 = function(_magitude) {
			var _dir = get_direction();
			x = lengthdir_x(_magitude, _dir);
			y = lengthdir_y(_magitude, _dir);
			return self;
		};
		...
		static interpolate_dir = function(_dir, _amount) {
			set_direction(
				iceberg.math.lerp_angle(get_direction(), _dir, _amount)
			);
			return self;
		};
		...
		static scale		   = function(_vect) {
			var _dot	= iceberg.vector.dot(self, _vect) 
