	...
	#region duration
		...
		// public
		duration_get_base	   = function() { 
			return __.duration_base;	
		};
		duration_get_current   = function() {
			return __.duration;
		};
		duration_get_progress  = function() {
			if (__.duration_base == 0) {
				return 0;	
			}
			return __.duration / __.duration_base;
		};
		duration_set_base	   = function(_duration) {
			__.duration_base = _duration;
			return self;
		};
		duration_set_current   = function(_duration) {
			__.duration = _duration;
			return self;
		};
		duration_reset_to_base = function() {
			__.duration = __.duration_base;
			return self;
		};
		
		// private
		with (__) {
			duration_tick =  method(_self, function() {
				if (__.duration  > 0) __.duration--;	
				if (__.duration == 0) destroy();	
			});
		...
		with (__) {
			...
			stick_update_pos		 =  method(_self, function() {
				if (__.stick_to_owner_pos) {
					var _x = __.stick_owner.x + (__.stick_owner_x_offset * __.stick_owner_facing);
					var _y = __.stick_owner.y + (__.stick_owner_y_offset);
					position_set_x(_x); x = _x;
					position_set_y(_y); y = _y;
				}
			});
			stick_owner				 =	_self[$ "stick_owner"		   ] ?? _self.get_owner();
			stick_to_owner_pos		 =	_self[$ "stick_to_owner_pos"   ] ?? true;
			stick_to_owner_angle	 =	_self[$ "stick_to_owner_angle" ] ?? true;
			stick_to_owner_facing	 =	_self[$ "stick_to_owner_facing"] ?? true;
	...
	#region collision
		...
		collision_assign_data	= function(_collision_objects) {

			for (var _i = 0, _len_i = array_length(_collision_objects); _i < _len_i; _i++) {
			
				var _data = _collision_objects[_i];
				
				collision_object_add(_data.index, _data.on_start, _data.on_stop, {
				...
				if (_data[$ "filter"] != undefined) {
					...
					collision_filter_set(_data.index, _filter_name, _filter_method);
				}
			};
		}
